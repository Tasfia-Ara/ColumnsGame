...
respond_to_A:
    ...
    check_left_done:
        ...
        li $t7, 0x000000    # Store black
        
        # Check top block collision to see if theres a block to the left
        subi $t6, $t9, 4 # Gets block to the left of our top block
        lw $t8, 0($t6) # Gets colour at that left block
        bne $t8, $t7, dont_move  # If it's not black, don't move
        
        # Check middle block left collision 
        addi $t6, $t9, 124  # $t9 + 128 - 4
        lw $t8, 0($t6)
        bne $t8, $t7, dont_move  # If not black, there's a block, can't move
        
        # Check bottom block left collision
        addi $t6, $t9, 252  # $t9 + 256 - 4
        lw $t8, 0($t6)
        bne $t8, $t7, dont_move  # If not black, there's a block, can't move
...
respond_to_D:
    ...
    check_right_done:
        bne $t8, $zero, dont_move           # Check if flag equals to 1, if so redraw pixels in the same spot through dont_move function
         ...
         # Check top block collision to see if theres a block to the right
        addi $t6, $t9, 4 # Gets block to the left of our top block
        lw $t8, 0($t6) # Gets colour at that left block
        bne $t8, $t7, dont_move  # If it's not black, don't move
        
        # Check middle block right collision 
        addi $t6, $t9, 132  # $t9 + 128 + 4
        lw $t8, 0($t6)
        bne $t8, $t7, dont_move  # If not black, there's a block, can't move
        
        # Check bottom block right collision
        addi $t6, $t9, 260  # $t9 + 256 + 4
        lw $t8, 0($t6)
        bne $t8, $t7, dont_move  # If not black, there's a block, can't move
